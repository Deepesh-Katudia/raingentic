...
export class ChainWatcher {
  ...
  start(): void {
    ...
    this.unwatch = this.client.watchContractEvent({
      ...
      // Forces plain eth_getLogs polling instead of eth_newFilter — the public
      // testnet RPC doesn't reliably support filters, which silently drops logs.
      poll: true,
      ...
      onLogs: (logs) => {
        ...
        for (const log of logs) {
          ...
          this.store.emit({
            ...
            // Contract timestamp is block.timestamp — seconds, not ms.
  ...
  /** Public so a real income event (e.g. a weekly fund transfer) can force an
   *  immediate re-read instead of waiting up to pollIntervalMs for the card
   *  limit to catch up. */
